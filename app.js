// ===================================================================
// SUPER LEAGUE — app.js
// Performance-optimized: deferred Firebase init, active-page-only rendering.
// ===================================================================
...
// Defer Firebase init until DOM is ready and libs are loaded.
let db = null;
...
// Hide the initial loader once first data render completes.
...
function saveMatchEventsData() {
    ...
    Object.entries(playerGoalMap).forEach(([pId, goals]) => {
        ...
        m.events.push({
            playerId: pId,
            playerName: scorerPlayer ? scorerPlayer.name : null,
            teamId: snapshotTeamId,
            type: 'goals',
            count: goals.length,
            scoringType: goals[0].scoringType,
            goalsList: goals,
...
function renderDashboardAll() {
    ...
    if (activeAppPageId === 'dash') {
        ...
        if (isSeasonComplete()) {
            ...
            let champion = teams.filter(t => t.id !== 'dummy-bye')[0];
...
// --- BOOTSTRAP ---
// Set copyright year immediately (no dependency on Firebase).
document.getElementById("copyrightYear").innerText = new Date().getFullYear();

// Wait for deferred Firebase libs to load, then init.
// Using a small poll since defer scripts execute in order but we need
// the global `firebase` symbol to be available.
function waitForFirebaseLibs() {
    if (typeof firebase !== 'undefined' && firebase.initializeApp) {
        initFirebase();
    } else {
        setTimeout(waitForFirebaseLibs, 50);
    }
}
waitForFirebaseLibs();

// Safety net: if Firebase never connects (offline / blocked), hide the
// loader after 8s so visitors aren't stuck staring at a spinner.
